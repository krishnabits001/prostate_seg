...
class f1_utilsObj:
    ...
    def calc_pred_sf_mask(self, sess, ae, labeled_data_imgs, axis_no=2):
        ...
        for slice_no in range(total_slices):
            ...
            if (slice_no == 0):
                ...
                mergedlist_y_pred = np.reshape(seg_pred, (1,img_size_x, img_size_y, self.num_classes))
            else:
                #seg_pred_final = np.reshape(seg_pred, (1,self.img_size_x, self.img_size_y, self.num_classes))
                seg_pred_final = np.reshape(seg_pred, (1,img_size_x, img_size_y, self.num_classes))
    ...
    def pred_segs_prostate_test_subjs(self, sess,ae, save_dir,orig_img_dt,test_list,struct_name,print_assd_hd_scores=0):
        '''
        ...
        input params:
            ...
            orig_img_dt: dataloader of prostate data
    ...
    def plot_predicted_seg_ss(self, test_data_img,test_data_labels,predicted_labels,save_dir,test_id):
        ...
        for example_i in range(n_examples):
            ...
            axs[0][example_i].imshow(test_data_img[:,:,(example_i+1)*4],cmap='gray')
            axs[1][example_i].imshow(test_data_labels[:,:,(example_i+1)*4])
            axs[2][example_i].imshow(np.squeeze(predicted_labels[:,:,(example_i+1)*4]))
